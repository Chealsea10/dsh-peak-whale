...
import { renderOffPeak, renderStatus, type OffPeakValue, type StatusValue } from './render.js'
...
export function apply(ctx: Context, config: Config) {
  ...
  let pricing: ResolvedPricing = resolvePricing(config)
  let source = () => config
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    if (typeof settings.installSection !== 'function') {
      console.warn(
        '[dsh-peak-whale] this host\'s settings service has no installSection (dsh 0.1.0-rc.x): ' +
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Show the current DeepSeek API pricing period: peak vs half-price off-peak, the price ' +
      ...
      async execute(args, exec): Promise<StatusValue> {
        const now = new Date()
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'List the upcoming off-peak (half-price) windows for the DeepSeek API as UTC intervals. ' +
      ...
      async execute(args): Promise<OffPeakValue> {
        ...
        return {
          ...
          hint:
            "Schedule work inside a window with schedule_create: { kind: 'at', at: windows[i].startUtc, prompt: '...' }. " +
...
async function balanceValue(config: Config, signal: AbortSignal): Promise<StatusValue['balance']> {
  if (!config.apiKey) {
